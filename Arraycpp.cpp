...
//destructor
 ...
 //dont need to pass now struct Array *arr
 void Display();
 void Append(int x);
 ...
 int isSorted();
 void Rearrange();
 //from: struct Array *Merge( struct Array *arr1, struct Array *arr2)
...
// classname-> Array ; ::-> scope resolution
void Array::Display()
{
 int i;
 cout<<"\nElements are\n";
...
{
//change from arr->length to just length
 if(length<size)
 A[length++]=x;
